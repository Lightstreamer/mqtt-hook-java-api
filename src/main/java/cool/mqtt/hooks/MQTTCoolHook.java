...
public interface MQTTCoolHook {
   ...
   * @throws HookException if the the initialization can't complete successfully; in this case the
   *         MQTT&#46;Cool server process will abort
   */
   ...
   * provided by the client while connecting to MQTT&#46;Cool, if no other static entries have been
   * provided in the <code>mqtt_master_connector_conf.xml</code> file for the given alias.
   *
   * @param connectionAlias the connection alias provided by a client to address a specific MQTT
   *        broker
   * @return an {@code MqttBrokerConfig} object, or {@code null} if this Hook can not supply a valid
   *         MQTT broker configuration
   * @throws HookException if this Hook runs against a specific issue while providing an MQTT broker
   *         configuration
   */
   ...
   * with the client principal, when a client certificate is specified. Note that the Lightstreamer
   * server must be configured appropriately to receive the client principal (see
   * <code>use_client_auth</code> and <code>force_client_auth</code> parameters).
   *
   * @param sessionId the unique identifier of client session
   * @param user the username of the user trying to opening a session; it can be {@code null} if no
   *        username has been provided
   * @param password the password of the user trying to opening a session; it can be {@code null} if
   *        no password has been provided
   * @param clientContext the key-value map which contains the properties of the client request;
   *        available keys are:
   *        <ul>
   *        <li><code>"REMOTE_IP"</code> - string representation of the remote IP related to the
   *        current connection; it may be a proxy address.
   *        <li><code>"REMOTE_PORT"</code> - string representation of the remote port related to the
   *        current connection.
   *        <li><code>"USER_AGENT"</code> - the user-agent as declared in the current connection
   *        HTTP header.
   *        <li><code>"FORWARDING_INFO"</code> - the comma-separated list of addresses forwarded by
   *        intermediaries, obtained from the <code>X-Forwarded-For HTTP</code> header, related to
   *        the current connection; intermediate proxies usually set this header to supply
   *        connection routing information. Note that if the number of forwards to be considered
   *        local to the Server environment has been specified through the
   *        <code>skip_local_forwards</code> configuration element, in order to better determine the
   *        remote address, then these forwards will not be list.
   *        <li><code>"LOCAL_SERVER"</code> - the name of the specific server socket that handles
   *        the current connection, as configured through the <code>http_server</code> or
   *        <code>https_server</code> element.
   *        <li><code>"HTTP_HEADERS"</code> - a map object that contains a name-value pair for each
   *        header found in the HTTP request that originated the call.
   *        </ul>
   * @param clientPrincipal the identification name reported in the client TLS/SSL certificate
   ...
   *        <ul>
   *        <li>For a <i>dedicated connection</i>, it corresponds to the actual client identifier
   *        being used to connect to the target MQTT broker.</li>
   *        <li>For a <i>shared connection</i>, it is an empty ("") string, as the actual client
   *        identifier is determined through the connection parameters specified in the
   *        <code>mqtt_master_connector_conf.xml</code> file, or by the {@code MqttBrokerConfig}
   *        instance provided by the {@link #resolveAlias(String)} method.</li>
   *        </ul>
   ...
   * 
   * @param sessionId the unique identifier of the client session
   * @param clientId the client identifier as sent by the client (and detailed in
   *        {@link #canConnect})
   * @param brokerAddress the address of the MQTT broker disconnected from
   */
   ...
   *         checks
   */
  boolean canPublish(String sessionId, String clientId, String brokerAddress, MqttMessage message)
   ...
   *        {@link #canConnect})
   * @param brokerAddress the address of the MQTT broker connected to
   * @param topicFilter the topic filter unsubscribed from
   */
